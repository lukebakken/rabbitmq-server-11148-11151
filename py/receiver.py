...
class Recv(MessagingHandler):
    ...
    def on_link_opened(self, event):
        print(
            "   -I- RECEIVE: receiver link opened ",
            event.receiver,
            " for source address '{0}'".format(event.receiver.source.address),
        )
    ...
    def on_link_closing(self, event):
        print(
            "   -I- Closing link receiver for target address '{0}'".format(
    ...
    def on_message(self, event):
        print(
            "   -D- credit=",
            event.receiver.credit,
            "  queued (deliveries on this link)=",
            event.receiver.queued,
        )  # event.receiver.drained
        # print(event.message).
        if DEBUG == "ON":
            ...
            print(
                "   -D-    event.message.correlation_id=", event.message.correlation_id
            )
        ...
        if self.expected == 0 or self.received < self.expected:
            ...
            )  # auto_accept=False  by defaut auto ACCEPTED after return on on_message() event handler.
...
try:
    ...
    print(
        "Starting client receiver '" + container.container_id + "' with link receiver",
        opts.name,
        " waiting ",
        opts.messages,
        "messages",
