...
class Send(MessagingHandler):
    ...
    def on_start(self, event):
        ...
        cur_sender = event.container.create_sender(
            context=cur_conn, target=cur_address, name=self.sender_name
        )  # target for sender
    ...
    def on_link_opened(self, event):
        print(
            "   -I- SEND: sender link opened",
            event.sender,
            " for target address '{0}'".format(event.sender.target.address),
        )
    ...
    def on_connection_error(self, event):
        ...
        print(
            "Transport failure for amqp broker:",
            self.url,
            "Error:",
    ...
    def on_link_closed(self, event):
        print(
            "   -I- Link sender is closed for target address '{0}'".format(
    ...
    def on_link_closing(self, event):
        print(
            "   -I- Closing link sender for target address '{0}'".format(
                event.sender.target.address
            )
        )
    ...
    def on_link_error(self, event):
        print(
            "   -E- Error link sender for target address '{0}'".format(
                event.sender.target.address
            )
        )
    ...
    def on_transport_error(self, event):
        print(
            "   -E- Transport error in the AMQP connection. This includes authentication errors as well as socket errors."
        )
    ...
    def on_sendable(
        self, event
    ):  # max 60s timeout to send messages correspomding to credits available
        ...
        while event.sender.credit and self.sent < self.total:
            ...
            msg = Message(
                ...
                user_id=bytes("test", "ascii", errors="strict"),
                subject="test message",
                content_type="application/json",
                #                        ttl = 200, # in secondes   #use ttl instead of expiry_time
                #                        subject = "test message",
            )
            cid_delivery = event.sender.send(
                msg
            )  # is an asynchronous call,. The return of the call does not indicate that the messages has been transferred yet.
            print(
                "Send msg id=" + str(self.sent + 1) + "    Delivery" + str(cid_delivery)
            )
            self.sent += 1
...
parser = optparse.OptionParser(
    usage="usage: %prog [options]", description="Send messages to the supplied address."
)
parser.add_option(
    "-a",
    "--address",
    default="localhost:5672/exampleQueue",
...
parser.add_option(
    "-n", "--name", default="sender1", help="name of the Sender link (default %default)"
)
parser.add_option(
    "-w",
    "--wait",
...
try:
    ...
    print(
        "Starting client sender'" + container.container_id + "' with link sender",
        opts.name,
        " sending ",
        opts.messages,
        "messages",
